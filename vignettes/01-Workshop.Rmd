...
- Data structures like `data.frame` help manage data
...
- Working with `SummarizedExperiment` for data management
- Using annotation packages to map between gene identifiers
...
- Map from Ensembl identifiers to gene symbol
...
```{r, message=FALSE}
as_tibble(colData(tcell), rownames = "cell") %>%
...
- ...Or another -- add reduced dimension information to `fig1`
...
- _Bioconductor_ data structures like `SummarizedExperiment` and
  `MultiAssayExperiment` help substantially with these data managment
  tasks.
