---
title: "A.3 -- Data Input and Manipulation"
...
output:
  BiocStyle::html_document:
    ...
    toc_depth: 2
vignette: >
  % \VignetteIndexEntry{A.3 -- Data Input and Manipulation}
...
0. (Easy) Use _RStudio_'s "Import Dataset" function to find the
   'BRFSS-subset.csv' file that you've downloaded to your local
   disk. Skip the next two steps.

1. (Alternative) Use `file.choose()` to find the path to the file
   'BRFSS-subset.csv'
...
<!--
    ...
    path <- system.file(package="RIntro.RPCI.Jan2017", "extdata", "BRFSS-subset.csv")
    ```
-->

2. (Alternative, continued) Input the data using `read.csv()`,
...
6. (advanced) Use a `formula` and the `aggregate()` function to
...
Either use _RStudio_'s "Import dataset" (with a header, use first
column for row names, comma-separated) or choose the file that
contains ALL (acute lymphoblastic leukemia) patient information and
input the date using `read.csv()`; for `read.csv()`, use the
`row.names=1` to use the first column as row names.
...
<!--
```{r echo=FALSE}
path <- system.file(package="RIntro.RPCI.Jan2017", "extdata", "ALL-phenoData.csv")
```
-->
