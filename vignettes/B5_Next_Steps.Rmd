...
1. Correct: consistent with hand-worked examples (`identical()`,
   `all.equal()`)
...
1. Profile
    ...
    - _Time_ evaluation of select lines or simple chunks of code with
      `system.time()` or the [microbenchmark][] package.
...
5. Re-use existing, tested code
    ...
    - Efficient domain-specific implementations, e.g., [snpStats][]
      for GWAS linear models; [limma][] for microarray linear models;
      [edgeR][], [DESeq2][] for negative binomial GLMs relevant to
      RNASeq.
    - Reuse others' work -- [DESeq2][], [GenomicRanges][],
...
This small example motivates the use of parallel execution and
demonstrates how `bplapply()` can be a drop in for `lapply`.

Use `system.time()` to explore how long this takes to execute as `n`
increases from 1 to 10. Use `identical()` and [microbenchmark][] to
compare alternatives `f0()` and `f1()` for both correctness and
performance.

`fun` sleeps for 1 second, then returns `i`.

```{r parallel-sleep, eval=FALSE}
library(BiocParallel)
...
[lineprof]: https://cran.r-project.org/package=lineprof
