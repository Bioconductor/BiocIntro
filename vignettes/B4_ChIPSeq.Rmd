...
output:
  BiocStyle::html_document2:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{B4 -- ChIP-seq}
...
- Key references
    ...
    - Kharchenko, Tolstorukov, and Park ([2008][]).
...
This exercise is based on the [csaw][] vignette, where more detail can
be found.

## Steps 1 - 4: Experimental Design ... Alignment
...
using [Rsubread][], and post-processed (sorted and indexed) using
[Rsamtools][] with the script available at
...
For further discussion about the reduction step, see Chapter 2 of the
[csaw vignette][].

[csaw vignette]: https://bioconductor.org/packages/release/bioc/vignettes/csaw/inst/doc/csawUserGuide.pdf
...
using the [edgeR][] TMM (trimmed median of M values) method seen in
the RNASeq differential expression lab. Explore vignette chapter 4 for
more on normalization (this is a useful resource when seeking to
develop normalization methods for other protocols!).
...
Differential binding will be assessed using [edgeR][], where we need
to specify the experimental design

```{r csaw-experimental-design}
design <- model.matrix(~Treatment, colData(data))
```

Apply a standard [edgeR][] work flow to identify differentially bound
regions. Creatively explore the results.
...
Finally, create a `GRanges` object that summarized the merged windows
and combined test statistics.

```{r csaw-combined-merged}
final <- merged$region
...
The 'top table' of most differentially expressed regions can be
obtained by ordering `final` by the `PValue` column, perhaps first
filtering to remove regions that do not overlap known promoters.
